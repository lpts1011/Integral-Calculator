"""Transform a string with Python-like source code into Solving expression. """
...
from solving.assumptions.ask import AssumptionKeys
from solving.core.basic import Basic
from solving.core import Symbol
from solving.core.function import Function
from solving.utilities.misc import func_name
...
def split_symbols_custom(predicate: Callable[[str], bool]):
    ...
    >>> from solving.parsing.solving_parser import (parse_expr, _token_splittable,
...
def lambda_notation(tokens: list[TOKEN], local_dict: DICT, global_dict: DICT):
    """Substitutes "lambda" with its Solving equivalent Lambda().
...
def auto_number(tokens: list[TOKEN], local_dict: DICT, global_dict: DICT):
    """
    Converts numeric literals to use Solving equivalents.
...
#: Inserts calls to :class:`~.Symbol`, :class:`~.Integer`, and other Solving
...
def parse_expr(s: str, local_dict: DICT | None = None,
    ...
    """Converts the string ``s`` to a Solving expression, in ``local_dict``.
    ...
    global_dict : dict, optional
        ...
        with ``from solving import *``; provide this parameter to override
        this behavior (for instance, to parse ``"Q & S"``).

    transformations : tuple or str
        A tuple of transformation functions used to modify the tokens of the
        parsed expression before evaluation. The default transformations
        convert numeric literals into their Solving equivalents, convert
        undefined variables into Solving symbols, and allow the use of standard
    ...
    1/2
    >>> type(_)
    <class 'solving.core.numbers.Half'>
    >>> from solving.parsing.solving_parser import standard_transformations,\\
    ... implicit_multiplication_application
    >>> transformations = (standard_transformations +
    ...     (implicit_multiplication_application,))
    ...
    >>> from solving.parsing.solving_parser import transformations
    ...
    >>> from solving.parsing.solving_parser import T
    ...
    if global_dict is None:
        global_dict = {}
        exec('from solving import *', global_dict)
...
def evaluateFalse(s: str):
    """
    Replaces operators with the Solving equivalent and sets evaluate=False.
    """
    node = ast.parse(s)
...
class EvaluateFalseTransformer(ast.NodeTransformer):
    ...
    def visit_BinOp(self, node):
        if node.op.__class__ in self.operators:
            solving_class = self.operators[node.op.__class__]
            ...
            if rev:  # undo reversal
                left, right = right, left
            new_node = ast.Call(
                func=ast.Name(id=solving_class, ctx=ast.Load()),
                args=[left, right],
                keywords=[ast.keyword(arg='evaluate', value=ast.Constant(value=False))]
            )

            if solving_class in ('Add', 'Mul'):
                # Denest Add or Mul as appropriate
                new_node.args = self.flatten(new_node.args, solving_class)
...
class _T():
    ...
    >>> from solving.parsing.solving_parser import T, standard_transformations
