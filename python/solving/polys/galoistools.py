...
from solving.core.random import uniform, _randint
from solving.external.gmpy import SYMPY_INTS, MPZ, invert
from solving.polys.polyconfig import query
from solving.polys.polyerrors import ExactQuotientFailed
...
def linear_congruence(a, b, m):
    ...
    """
    from solving.polys.polytools import gcdex
    if a % m == 0:
        if b % m == 0:
            return list(range(m))
        else:
...
def _csolve_prime_las_vegas(f, p, seed=None):
    ...
    """
    from solving.polys.domains import ZZ
    from solving.ntheory import sqrt_mod
