...
from solving.polys.polyerrors import GeneratorsError, OptionError, FlagError
from solving.utilities import numbered_symbols, topological_sort, public
from solving.utilities.iterables import has_dups, is_sequence
...
class Options(dict):
    ...
    * Series --- boolean flag
    ...
    gens: tuple[Expr, ...]
    domain: solving.polys.domains.Domain
...
class Order(Option, metaclass=OptionType):
    ...
    @classmethod
    def default(cls):
        return solving.polys.orderings.lex

    @classmethod
    def preprocess(cls, order):
        return solving.polys.orderings.monomial_key(order)
...
class Domain(Option, metaclass=OptionType):
    ...
    def preprocess(cls, domain):
        if isinstance(domain, solving.polys.domains.Domain):
            return domain
        elif hasattr(domain, 'to_domain'):
            return domain.to_domain()
        elif isinstance(domain, str):
            if domain in ['Z', 'ZZ']:
                return solving.polys.domains.ZZ

            if domain in ['Q', 'QQ']:
                return solving.polys.domains.QQ

            if domain == 'ZZ_I':
                return solving.polys.domains.ZZ_I

            if domain == 'QQ_I':
                return solving.polys.domains.QQ_I

            if domain == 'EX':
                return solving.polys.domains.EX
            ...
            if r is not None:
                ...
                if prec is None:
                    return solving.polys.domains.RR
                else:
                    return solving.polys.domains.RealField(int(prec))
            ...
            if r is not None:
                ...
                if prec is None:
                    return solving.polys.domains.CC
                else:
                    return solving.polys.domains.ComplexField(int(prec))

            r = cls._re_finitefield.match(domain)

            if r is not None:
                return solving.polys.domains.FF(int(r.groups()[1]))
            ...
            if r is not None:
                ...
                if ground in ['Z', 'ZZ']:
                    return solving.polys.domains.ZZ.poly_ring(*gens)
                elif ground in ['Q', 'QQ']:
                    return solving.polys.domains.QQ.poly_ring(*gens)
                elif ground in ['R', 'RR']:
                    return solving.polys.domains.RR.poly_ring(*gens)
                elif ground == 'ZZ_I':
                    return solving.polys.domains.ZZ_I.poly_ring(*gens)
                elif ground == 'QQ_I':
                    return solving.polys.domains.QQ_I.poly_ring(*gens)
                else:
                    return solving.polys.domains.CC.poly_ring(*gens)
            ...
            if r is not None:
                ...
                if ground in ['Z', 'ZZ']:
                    return solving.polys.domains.ZZ.frac_field(*gens)
                else:
                    return solving.polys.domains.QQ.frac_field(*gens)

            r = cls._re_algebraic.match(domain)

            if r is not None:
                gens = list(map(sympify, r.groups()[1].split(',')))
                return solving.polys.domains.QQ.algebraic_field(*gens)
    ...
    def postprocess(cls, options):
        if 'gens' in options and 'domain' in options and options['domain'].is_Composite and \
            ...
            raise GeneratorsError(
                ...
                'domain' in options and options['domain'] == solving.polys.domains.EX:
...
class Gaussian(BooleanOption, metaclass=OptionType):
    ...
    def postprocess(cls, options):
        if 'gaussian' in options and options['gaussian'] is True:
            options['domain'] = solving.polys.domains.QQ_I
...
class Extension(Option, metaclass=OptionType):
    ...
    def postprocess(cls, options):
        if 'extension' in options and options['extension'] is not True:
            options['domain'] = solving.polys.domains.QQ.algebraic_field(
                *options['extension'])
...
class Modulus(Option, metaclass=OptionType):
    ...
    def postprocess(cls, options):
        if 'modulus' in options:
            ...
            options['domain'] = solving.polys.domains.FF(modulus, symmetric)
