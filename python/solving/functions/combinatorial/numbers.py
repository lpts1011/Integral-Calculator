...
from solving.core import S, Symbol, Add, Dummy
from solving.core.cache import cacheit
from solving.core.containers import Dict
from solving.core.expr import Expr
from solving.core.function import ArgumentIndexError, DefinedFunction, expand_mul
from solving.core.logic import fuzzy_not
from solving.core.mul import Mul
from solving.core.numbers import E, I, pi, oo, Rational, Integer
from solving.core.relational import Eq, is_le, is_gt, is_lt
from solving.external.gmpy import SYMPY_INTS, remove, lcm, legendre, jacobi, kronecker
...
from solving.ntheory.factor_ import (factorint, _divisor_sigma, is_carmichael,
                                   find_carmichael_numbers_in_range, find_first_n_carmichaels)
from solving.ntheory.generate import _primepi
from solving.ntheory.partitions_ import _partition, _partition_rec
from solving.ntheory.primetest import isprime, is_square
from solving.polys.appellseqs import bernoulli_poly, euler_poly, genocchi_poly
from solving.polys.polytools import cancel
from solving.utilities.enumerative import MultisetPartitionTraverser
from solving.utilities.exceptions import sympy_deprecation_warning
from solving.utilities.iterables import multiset, multiset_derangements, iterable
from solving.utilities.memoization import recurrence_memo
...
class fibonacci(DefinedFunction):
    ...
    * ``fibonacci(n, x)`` gives the `n^{th}` Fibonacci polynomial in `x`, `F_n(x)`
    ...
    def _eval_rewrite_as_tractable(self, n, **kwargs):
        from solving.functions import sqrt, cos
        return (S.GoldenRatio**n - cos(S.Pi*n)/S.GoldenRatio**n)/sqrt(5)

    def _eval_rewrite_as_sqrt(self, n, **kwargs):
        from solving.functions.elementary.miscellaneous import sqrt
...
class tribonacci(DefinedFunction):
    ...
    * ``tribonacci(n, x)`` gives the `n^{th}` Tribonacci polynomial in `x`, `T_n(x)`
    ...
    def _eval_rewrite_as_sqrt(self, n, **kwargs):
        from solving.functions.elementary.miscellaneous import cbrt, sqrt
...
class bernoulli(DefinedFunction):
    ...
    * ``bernoulli(s, a)`` gives the generalized Bernoulli function
    ...
    def _eval_rewrite_as_zeta(self, n, x=1, **kwargs):
        from solving.functions.special.zeta_functions import zeta
...
class harmonic(DefinedFunction):
    ...
    * ``harmonic(n, m)`` gives the nth generalized harmonic number
    ...
    def _eval_rewrite_as_polygamma(self, n, m=S.One, **kwargs):
        from solving.functions.special.gamma_functions import gamma, polygamma
    ...
    def _eval_rewrite_as_zeta(self, n, m=S.One, **kwargs):
        from solving.functions.special.zeta_functions import zeta
        from solving.functions.special.gamma_functions import digamma
        ...
        if m == S.One:
            ...
            if n.is_Rational:
                ...
                if u.is_nonnegative and p.is_positive and q.is_positive and p < q:
                    from solving.functions.elementary.exponential import log
                    from solving.functions.elementary.integers import floor
                    from solving.functions.elementary.trigonometric import sin, cos, cot
                    k = Dummy("k")
    ...
    * ``euler(s, a)`` gives the generalized Euler function `\operatorname{E}(s, a)`.
    ...
    def eval(cls, n, x=None):
        ...
        # Euler polynomials
            ...
            from solving.core.evalf import pure_complex
            n = int(n)
...
class catalan(DefinedFunction):
    ...
    * ``catalan(n)`` gives the `n^{th}` Catalan number, `C_n`
    ...
    @classmethod
    def eval(cls, n):
        from solving.functions.special.gamma_functions import gamma
...
class genocchi(DefinedFunction):
    ...
    def _eval_rewrite_as_dirichlet_eta(self, n, x=1, **kwargs):
        from solving.functions.special.zeta_functions import dirichlet_eta
...
class andre(DefinedFunction):
    ...
    *alternating permutations* on `n` elements, where a permutation is alternating
    ...
    def _eval_rewrite_as_polylog(self, s, **kwargs):
        from solving.functions.special.zeta_functions import polylog
...
def nD(i=None, brute=None, *, n=None, m=None):
    ...
    from solving.functions.special.polynomials import laguerre
    from solving.abc import x
    def ok(x):
